...
**Document Ingestion:**
```
File Upload → Docling Parser → Recursive Chunker (10% overlap) → 
JSON Storage → PostgreSQL (nomic-embed-text embeddings)
```

**RAG Query:**
```
User Query → Embedding → Vector Search (top-K) → 
Context Retrieval → Researcher Agent → Finisher Agent → 
...
# Test connection
docker exec -it postgres_db psql -U rag_user -d rag_db -c "SELECT 1;"
```
...
- Vector dimensions fixed at 768 (nomic-embed-text)
- Changing embedding model requires database rebuild
- Python 3.11+ required
- Uses `uv` package manager
- All documents indexed in single collection
